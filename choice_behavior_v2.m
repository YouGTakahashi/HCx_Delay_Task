...
%average and SEM;
avg_Ctrl_F(1,1) = mean(Ctrl(:,1)); %First
avg_Ctrl_L(1,1) = mean(Ctrl(:,2)); %Last

avg_HCx_F(1,1) = mean(HCx(:,1)); %First
avg_HCx_L(1,1) = mean(HCx(:,2)); %Last

sem_Ctrl_F(1,1) = std(Ctrl(:,1))/sqrt(length(Ctrl(:,1)));
sem_Ctrl_L(1,1) = std(Ctrl(:,2))/sqrt(length(Ctrl(:,2)));

sem_HCx_F(1,1) = std(HCx(:,1))/sqrt(length(HCx(:,1)));
sem_HCx_L(1,1) = std(HCx(:,2))/sqrt(length(HCx(:,2)));

%for scatter plot;
Ctrl_scat_F(1:length(Ctrl(:,1)),1) = rand(length(Ctrl(:,1)),1);
for a = 1:length(Ctrl(:,1));
    Ctrl_scat_F(a,1) = (Ctrl_scat_F(a,1)/10 - 0.05)+1;
end
Ctrl_scat_F(:,2) = Ctrl(:,1);
...
HCx_scat_F(1:length(HCx(:,1)),1) = rand(length(HCx(:,1)),1);
for a = 1:length(HCx(:,1));
    HCx_scat_F(a,1) = (HCx_scat_F(a,1)/10 - 0.05)+3.5;
end
HCx_scat_F(:,2) = HCx(:,1);
...
bar(1,avg_Ctrl_F,'FaceColor',[0 0 0]); hold on;
bar(2,avg_Ctrl_L,'FaceColor',[0 0 0]); hold on;
bar(3.5,avg_HCx_F,'FaceColor',[.5 .5 .5]); hold on;
bar(4.5,avg_HCx_L,'FaceColor',[.5 .5 .5]); hold on;

errorbar(1,avg_Ctrl_F,sem_Ctrl_F,'Color',[0 0 0],'LineStyle','none');hold on;
errorbar(2,avg_Ctrl_L,sem_Ctrl_L,'Color',[0 0 0],'LineStyle','none');hold on;
errorbar(3.5,avg_HCx_F,sem_HCx_F,'Color',[0 0 0],'LineStyle','none');hold on;
errorbar(4.5,avg_HCx_L,sem_HCx_L,'Color',[0 0 0],'LineStyle','none');hold on;

scatter(Ctrl_scat_F(:,1),Ctrl_scat_F(:,2),'MarkerEdgeColor',[1 0 0]); hold on;
scatter(Ctrl_scat_L(:,1),Ctrl_scat_L(:,2),'MarkerEdgeColor',[1 0 0]); hold on;
scatter(HCx_scat_F(:,1),HCx_scat_F(:,2),'MarkerEdgeColor',[1 0 0]); hold on;
